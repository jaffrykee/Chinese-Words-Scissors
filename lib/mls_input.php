		...
		for($i=0,$j=0,$k=0;$sa[$i]!=NULL;$i++)
		{
			if($j<=2)
			{
				if($j<=2)
				{
					...
					{
						$sql="SELECT * FROM mls_input_word_to_real WHERE word LIKE '".$str_now."%'";
		//	echo $sql;
						$w_s=get_all($sql);
						$p=count($w_s);
		//	echo ' | **********************************$p='.$p.'**********************************';
	...
	//						$str_last=$str_now;
		...
		/*					if($p==1)
					...
					echo $str_input.' | ';
								...
								{
									$sql_1="SELECT * FROM mls_input_word_to_real WHERE word_id='".$w_s['0']['id']."'";
					echo $sql_1.' | ';
							...
							else
							{
								for(;;)
								{
									$sql="SELECT * FROM mls_word WHERE word LIKE '".$sa[$i].$sa[$i+1]."%'";
									$res=mysql_query($sql);
									$w_s=array();
